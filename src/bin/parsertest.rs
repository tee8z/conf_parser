...
fn main() {
    ...
    let _ = processer::write_to_file(&lnd_conf, "./test_files/lnd_1.conf").unwrap();
    ...
    let _ = processer::write_to_file(&bitcoind_conf, "./test_files/bitcoind_1.conf").unwrap();
    ...
    let contents =File::open("./test_files/smaple_bitcoind_2.conf").expect("file read error");
    let bitcoind_conf = processer::read_to_file_conf(&contents).unwrap();
